...
const Deans4percentage = () => {
          ...
          if (facultyData && facultyData.program_id) {
            ...
            const yearLevelTwoStudentsCount = studentsData.reduce(
              (count, student) => {
                ...
                return yearLevel === 4 ? count + 1 : count;
              },
              0
            );
              ...
              const studentsWithAverageGradeForYearTwo =
                filteredStudentsList.filter((student) => {
                  return (
                    ...
                    student.averageGradesPerYear[4] !== undefined
                  );
                });
  const percentage =
    studentCount !== 0
      ? Math.round(
          (studentsWithAverageGradeForYearTwo.length / studentCount) * 100
        )
      : 0;
  ...
  const options = {
    ...
    labels: studentsList.map(
      (student) => `${student.first4number + 3} - ${student.first4number + 4}`
    ),
  };
...
export default Deans4percentage;
